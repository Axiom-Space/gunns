...
////////////////////////////////////////////////////////////////////////////////////////////////////
void GunnsElectConverterInput::step(const double)
{
...
void GunnsElectConverterInput::minorStep(const double dt __attribute__((unused)), const int minorStep __attribute__((unused)))
{
    if (mNodeMap[0] == getGroundNodeIndex()) {
        ...
        double current  = 0.0;
        double iuvLimit = static_cast<double>(mInputUnderVoltageTrip.getLimit());
        if (mEnabled and not (mOverloadedState or mInputOverVoltageTrip.isTripped()
            ...
            } else if (iuvLimit > DBL_EPSILON) {
                /// - If there is a non-zero IUV limit, then don't load the input circuit much
                ///   farther below it.  This avoids increasing our load too aggressively in
                ///   response to input voltage suddenly dropping to near zero, and causing the
                ///   input circuit to isolate us by mistake.
                current = scaledInputLoad / std::max(mInputVoltage, 0.99 * iuvLimit);
        ...
        mSourceVector[0] = std::min(0.0, -current);
    }
}
